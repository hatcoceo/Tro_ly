...
class VirtualAssistant:
    def __init__(self):
        ...
        self.handlers = [DeleteCommandHandler(self.km),
    ...
    def load_plugins_from_folder(self, folder="plugins"):
        ...
        for filename in os.listdir(folder):
            ...
            # Gọi hàm đăng ký nếu có
            if "register" in info and callable(info["register"]):
                try:
                    info["register"](self)
                    print(f"✅ Plugin {filename} đã đăng ký thành công.")
                except Exception as e:
                    print(f"⚠️ Plugin {filename} gặp lỗi khi gọi register: {e}")
