    ...
    def switch_version(self, version: str) -> bool:
        ...
        if self.version_manager.switch_version(version):
            ...
            for handler in self.handlers:
                if isinstance(handler, TeachCommandHandler) or isinstance(handler, DeleteCommandHandler) or \
                   isinstance(handler, SearchCommandHandler) or isinstance(handler, DocumentCommandHandler) or \
                   isinstance(handler, ListKnowledgeHandler):
                    handler.km = self.km
                elif isinstance(handler, EventCommandHandler) or isinstance(handler, DurationHandler) or \
                     isinstance(handler, AdvancedEventHandler) or isinstance(handler, ListEventHandler):
                    handler.em = self.em
