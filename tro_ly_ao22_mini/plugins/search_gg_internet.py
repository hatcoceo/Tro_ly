...
RSS_FEEDS = {'vn': 'https://vnexpress.net/rss/tin-moi-nhat.rss', 'bbc':
    'http://feeds.bbci.co.uk/news/rss.xml', 'cafebiz':
    'https://cafebiz.vn/rss/home.rss', 'dantri':
    'https://dantri.com.vn/rss/home.rss', 'gg': 'google'}
...
def build_google_rss(keyword: str) ->str:
    query = quote(keyword)
    return (
        f'https://news.google.com/rss/search?q={query}&hl=vi&gl=VN&ceid=VN:vi')
            ...
            try:
                ...
                for i, item in enumerate(root.findall('.//item')):
                    ...
                    if source_name == 'gg':
                        ...
                        real_source = (source_tag.text if source_tag is not
                            None else 'GOOGLE')
                    else:
                        real_source = source_name.upper()
                    seen_titles.add(norm_title)
                    articles.append({'title': title_text, 'link': link_text,
                        'source': real_source})
