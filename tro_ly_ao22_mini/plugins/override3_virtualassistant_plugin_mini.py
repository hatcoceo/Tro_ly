...
class IVersionManager(ABC):
    ...
    def register_method_version(self, class_name: str, method_name: str,
        version: str, method_ref: Callable, description: str='', mode: str=
        'replace') ->bool:
        pass

    @abstractmethod
    def register_class_version(self, class_name: str, version: str,
        class_ref: Any) ->bool:
        pass

    @abstractmethod
    def get_method_version(self, class_name: str, method_name: str, version:
        Union[str, List[str]]=None) ->Optional[Callable]:
        pass

    @abstractmethod
    def get_class_version(self, class_name: str, version: str=None) ->Optional[
        Any]:
        pass
...
class BaseVersionManager(IVersionManager):
    ...
    def __init__(self):
        self.versions = {'default': {'methods': {}, 'classes': {},
    ...
    def _ensure_version_exists(self, version: str) ->None:
        ...
        if version not in self.versions:
            self.versions[version] = {'methods': {}, 'classes': {},
                'description': ''}
    ...
    def register_method_version(self, class_name: str, method_name: str,
        ...
        if mode == 'replace' or method_name not in methods:
            methods[method_name] = {'function': method_ref, 'description':
                description}
            return True
        if mode == 'append':
            old_func = methods[method_name]['function']
            methods[method_name] = {'function': lambda *args, **kwargs: (
                old_func(*args, **kwargs), method_ref(*args, **kwargs)),
                'description':
                f"{methods[method_name]['description']} + {description}"}
            return True
        if mode == 'multi':
            current_func = methods.get(method_name, {}).get('function')
            funcs = [current_func] if current_func and not isinstance(
                current_func, list) else current_func or []
            funcs.append(method_ref)
            methods[method_name] = {'function': funcs, 'description':
                description}
            return True
    ...
    def get_method_version(self, class_name: str, method_name: str, version:
        Union[str, List[str]]=None) ->Optional[Callable]:
        versions = [version] if isinstance(version, str) else version or [self
            .current_version]
        funcs = []
        for ver in versions:
            method_info = self.versions.get(ver, {}).get('methods', {}).get(
                class_name, {}).get(method_name)
            if not method_info:
                continue
        ...
        if not funcs:
            return None
        return lambda *args, **kwargs: {ver: f(*args, **kwargs) for ver, f in
            funcs}
...
class Loader:
    ...
    @staticmethod
    def _load_classes_from_folder(folder: str, base_class: Optional[Type[T]
        ]=None, info_attribute: Optional[str]=None, class_filter: Optional[
        Callable[[Type], bool]]=None, post_process: Optional[Callable[[Type
        ], Any]]=None) ->List[Type[T]]:
            ...
            try:
                ...
                for name in dir(module):
                    ...
                    if isinstance(attr, type):
                        if base_class and not (issubclass(attr, base_class) and
    ...
    def load_version_manager(cls) ->IVersionManager:
        ...
        implementations = cls._load_classes_from_folder('version_managers',
            base_class=IVersionManager, info_attribute='version_manager_info')
        if implementations:
    ...
    def load_interfaces(cls, interface_registry: Dict[str, Type]) ->None:
        ...
        def register_interface(cls: Type) ->None:
            ...
            return None
        cls._load_classes_from_folder('interfaces', post_process=
            register_interface)
    ...
    def load_plugins(cls, assistant: Any) ->None:
        ...
        for filename in os.listdir('plugins'):
            ...
            try:
                spec = importlib.util.spec_from_file_location('plugin_module',
                    path)
            ...
            if callable(info.get('register')):
                try:
                    ...
                    print(
                        f"⚠️ Lỗi khi chạy plugin_info['register'] trong {filename}: {e}"
                        )
            for method in info.get('methods', []):
                assistant.version_manager.register_method_version(method[
                    'class_name'], method['method_name'], method['version'],
                    method['function'], method.get('description', ''),
                    method.get('mode', 'replace'))
                print(
                    f"🧩 Đăng ký method: {method['class_name']}.{method['method_name']} ({method['version']})"
                    )
            for cls in info.get('classes', []):
                assistant.version_manager.register_class_version(cls[
                    'class_name'], cls['version'], cls['class_ref'])
                print(
                    f"🏗️  Đăng ký class: {cls['class_name']} ({cls['version']})"
                    )
    ...
    def load_dynamic_assistants(cls) ->None:
        ...
        def run_assistant(cls: Type) ->None:
            ...
            return None
        cls._load_classes_from_folder('assistants', class_filter=lambda c: 
            hasattr(c, 'run') and callable(getattr(c, 'run')), post_process
            =run_assistant)
    ...
    def load_run_mode(cls, run_mode_name: str=None) ->Type:
        ...
        run_modes = cls._load_classes_from_folder('run', class_filter=lambda
            c: c.__name__.startswith('Run'))
        if not run_modes:
            return None
        if run_mode_name:
    ...
    @classmethod
    def load_process_input(cls, assistant: 'VirtualAssistantCore'
        ) ->'ProcessInput':
        """Load and select appropriate process input handler"""
        print('🔍 Đang nạp Process Input Handlers haha vui quá')
        implementations = cls._load_classes_from_folder('process_inputs',
            base_class=ProcessInput, info_attribute='process_input_info')
...
class VirtualAssistantCore:
    ...
    def call_method(self, class_name: str, method_name: str, *args, version:
        Union[str, List[str]]=None, **kwargs) ->Any:
        func = self.version_manager.get_method_version(class_name,
            method_name, version)
        if func:
            try:
                ...
                return None
        print(
            f'⚠️ Không tìm thấy {class_name}.{method_name} ({version or self.version_manager.current_version})'
            )
        return None
    ...
    def call_class_method(self, class_name: str, method_name: str, *args,
        ...
        if cls:
            ...
            print(
                f'⚠️ Không tìm thấy class {class_name} ({version or self.version_manager.current_version})'
                )
        return None
...
class StartVirtualAssistant:
    ...
    def __init__(self, version_manager: Optional[IVersionManager]=None,
        ...
        self.assistant = core_impl(version_manager, run_mode
            ) if core_impl else VirtualAssistantCore(version_manager, run_mode)

    def _prepare_environment(self) ->None:
        """Chuẩn bị môi trường bằng cách tạo các thư mục cần thiết"""
        for folder in ['version_managers', 'interfaces', 'plugins',
            'assistants', 'virtualassistantcore', 'process_inputs']:
            Loader.ensure_folder(folder)
    ...
    def _load_core_implementation(self) ->Optional[Type[VirtualAssistantCore]]:
        ...
        cores = Loader._load_classes_from_folder('virtualassistantcore',
            base_class=VirtualAssistantCore, info_attribute='core_info')
        if cores:
            print(
                f'✅ Sử dụng VirtualAssistantCore implement: {cores[0].__name__}'
                )
            return cores[0]
...
def register(assistant):
    print('🔁 Chuyển giao cho trợ lý Asi-2...')
    asi3 = StartVirtualAssistant()
    asi3.start()


plugin_info = {'name': 'Asi-3 Full Override', 'enabled': False, 'register':
    register}
