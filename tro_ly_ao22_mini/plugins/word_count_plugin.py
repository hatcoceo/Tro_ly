    ...
    def handle(self, command: str) ->None:
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                text = f.read()
            if no_number:
                words = re.findall('\\b[^\\W\\d_]+\\b', text, flags=re.UNICODE)
                print('📄 Chế độ: KHÔNG tính số')
            else:
                words = re.findall('\\b\\w+\\b', text, flags=re.UNICODE)
...
plugin_info = {'enabled': True, 'register': register, 'command_handle': [
    'count word.txt', 'count word.txt --no-number']}
