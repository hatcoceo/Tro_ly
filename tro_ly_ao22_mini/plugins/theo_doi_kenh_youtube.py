...
CHANNEL_IDS = ['UCu3q59AXNNDGxT9LUBQeELA', 'UCDJIGO_3ycJAKLPv7Yh396A',
    'UCLWrocDxNVocoPs5d_qCaOg', 'UCZQdpU9kfmwzYuKJWrb_1Hw',
...
def monitor_loop():
    ...
    load_last_videos()
    print(
        f'[YouTubeMonitor] Đang theo dõi {len(CHANNEL_IDS)} kênh, kiểm tra mỗi {CHECK_INTERVAL}s'
        )
    while True:
        for ch_id in CHANNEL_IDS:
            try:
                ...
                if last_id != video_id:
                    print(
                        f'[YouTubeMonitor] 🎬 Kênh {ch_id} có video mới: {title}'
                        )
                    webbrowser.open(url)
...
plugin_info = {'enabled': True, 'register': register, 'command_handle': None}
