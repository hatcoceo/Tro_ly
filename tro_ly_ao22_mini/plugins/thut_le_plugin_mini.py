# sd: hiển thị thụt lề thut_le.py
# sd2: phục hồi thụt lề thut_le_indent.py
import os
...
class IndentVisualizerHandler:
    ...
    def handle(self, command: str) ->bool:
        ...
        if command.startswith('hiển thị thụt lề') or command.startswith(
            'indent'):
            filepath = command.replace('hiển thị thụt lề', '').replace('indent'
                , '').strip()
            if not filepath:
            ...
            with open(filepath, 'r', encoding='utf-8') as f_in, open(
                ...
                for i, line in enumerate(f_in, 1):
                    ...
                    level = indent_count // tab_size + 1 # điều chỉnh mức độ đầu tiên 
