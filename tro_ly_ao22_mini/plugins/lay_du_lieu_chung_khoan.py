...
class CleanChartHandler:
    ...
    def get_candle_data(self, symbol: str, exchange: str, days: int = 15):
        try:
            # Dùng nguồn VCI (hoặc thử KBS nếu cần)
    ...
    def can_handle(self, command: str) -> bool:
        return command.startswith("chart") or command.startswith("tv") or command.startswith("getdata")

    def handle(self, command: str) -> None:
        try:
            parts = command.split()
            if len(parts) < 2:
                print("❗ Cách dùng: chart HPG          # mở TradingView + FireAnt")
                print("            getdata all         # lấy dữ liệu nến tất cả cổ phiếu")
                print("            getdata HPG         # lấy dữ liệu riêng một mã")
                return
            ...
            if sub_cmd == "all":
                ...
                if not df.empty:
                    ...
                    print(df[['symbol', 'exchange', 'time', 'open', 'close', 'volume']].to_string(index=False))
            ...
            df = self.get_candle_data(symbol, exchange, days=115)
            if not df.empty:
...
plugin_info = {
    "enabled": True,
    "register": register,
    "command_handle": ["chart", "tv", "getdata"]
}