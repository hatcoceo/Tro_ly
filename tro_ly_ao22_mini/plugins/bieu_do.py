...
class PlotHandler:
    ...
    def handle(self, command: str) ->None:
        try:
            ...
            if content.startswith('excel '):
                ...
                if len(parts) < 5:
                    raise ValueError(
                        'Cú pháp: plot excel file.xlsx <cột|header> start_row end_row'
                        )
                _, file_path, col_or_header, start_row, end_row = parts
                start_row = int(start_row)
                end_row = int(end_row)
                numbers = self.read_excel_column(file_path, col_or_header,
                    start_row, end_row)
                folder = self.build_folder('excel', file_path)
                self.plot(numbers, title=f'{file_path}_{col_or_header}',
                    folder=folder)
                return
    ...
    def plot(self, numbers, title='plot', folder='plots'):
        ...
        plt.axhline(y=avg, color='red', linestyle='--', linewidth=2, label=
            f'Trung bình = {avg:.2f}')
        n1 = numbers[0]
        plt.axhline(y=n1, color='blue', linestyle='-', linewidth=2, label=
            f'N1 = {n1:.2f}')
        last_value = numbers[-1]
        count_below_last = sum(1 for n in numbers if n < last_value)
        percent = count_below_last / len(numbers) * 100
        print(
            f'📉 Số điểm dưới điểm cuối ({last_value:.2f}): {count_below_last} ({percent:.1f}%)'
            )
        plt.axhline(y=last_value, color='green', linestyle='-.', linewidth=
            2, label=
            f'Last = {last_value:.2f} | dưới: {count_below_last} ({percent:.1f}%)'
            )
        mid_x = (len(numbers) + 1) / 2
        plt.scatter(mid_x, avg, color='red', s=120, zorder=5, edgecolors=
            'black')
        plt.text(mid_x, avg, f'{avg:.2f}', ha='center', va='bottom',
            fontsize=10, bbox=dict(facecolor='white', alpha=0.7))
        plt.text(len(numbers), last_value, f'{count_below_last} điểm dưới',
            ha='right', va='bottom', fontsize=10, bbox=dict(facecolor=
            'white', alpha=0.7))
        plt.title(title)
        plt.xlabel('N')
        plt.ylabel('P')
