...
class Loader:
        ...
        for filename in os.listdir(folder):
            if filename.endswith(".py"):
                ...
                try:
                    ...
                    info = getattr(module, "version_manager_info", None)
                    if info and not info.get("enabled", True):
                        print(f"⏭️ Bỏ qua VersionManager {filename} (disabled)")
                        continue
    ...
    @staticmethod  
    def load_interfaces(folder: str, interface_registry: Dict[str, Type]) -> None:  
        Loader.ensure_folder(folder)  
        for filename in os.listdir(folder):  
            if filename.endswith(".py"):  
                path = os.path.join(folder, filename)  
    ...
    @staticmethod  
    def load_plugins(folder: str, assistant: Any) -> None:  
        Loader.ensure_folder(folder)  
        for filename in os.listdir(folder):  
            if filename.endswith(".py"):  
                path = os.path.join(folder, filename)  
    ...
    @staticmethod  
    def load_dynamic_assistants(folder: str) -> None:  
        Loader.ensure_folder(folder)  
        ...
        for filename in os.listdir(folder):
            if filename.endswith(".py"):
                ...
                try:
                    ...
                    core_info = getattr(module, "core_info", None)
                    if core_info and not core_info.get("enabled", True):
                        print(f"⏭️ Bỏ qua core {filename} (disabled)")
                        continue
