...
class KnowledgeManager:
    ...
    def delete_by_keyword(self, keyword: str) -> str:
        ...
        lines = self._read_all_lines()
        exact_matches = [i for i, line in enumerate(lines) if line.split('||')[0].strip().lower() == keyword]
        if exact_matches:
            for i in sorted(exact_matches, reverse=True):
                del lines[i]
            self._write_all_lines(lines)
            return '🗑️ Đã xóa các dòng phù hợp'
        similar_matches = [(i, line) for i, line in enumerate(lines) if keyword in line.lower()]
        ...
        result.extend(f'{i + 1}. [Dòng {idx + 1}] {line}' for i, (idx, line) in enumerate(similar_matches))
    ...
    def _match_exact(self, query: str) -> List[str]:
        lines = self._read_all_lines()
        return [ans.strip() for line in lines if (parts := line.split('||', 1))[0].strip().lower() == query.lower() for ans in [parts[1]]]
    ...
    def get_answer(self, query: str) -> str:
        ...
        suggestions_text = '\n'.join(
            f"{i + 1}. {line.split('||')[0].strip()}" for i, line in enumerate(self.suggestions)
        )
...
class KnowledgeHandler:
    ...
    def can_handle(self, command: str) -> bool:
        return command.startswith(('ho:', 'thêm:', 'xem:', 'sửa:', 'xóa:', 'xóa_chọn:'))

    def handle(self, command: str) -> str:
        cmd = command.lower().strip()
        if self.awaiting_choice and cmd.isdigit():
            self.awaiting_choice = False
            return self.km.get_answer_by_index(int(cmd))

        if cmd.startswith('thêm:'):
            return self.km.add(cmd[5:])
        elif cmd == 'xem:':
            return self.km.read()
            ...
            if len(parts) == 2:
                try:
                    ...
                    return self.km.update(line_num, parts[1].strip())
                except ValueError:
                    return '❌ Số dòng phải là số nguyên'
            else:
                return '❌ Cú pháp: sửa:<số dòng>:<nội dung mới>'
        elif cmd.startswith('xóa:'):
            return self.km.delete_by_keyword(cmd[4:])
        elif cmd.startswith('xóa_chọn:'):
            try:
                num = int(cmd[9:].strip())
                return self.km.delete_by_index(num)
            except ValueError:
                return '❌ Số thứ tự phải là số nguyên'
        elif cmd.startswith('ho:'):
            ...
            return '🤷‍♂️ Không hiểu yêu cầu CRUD'


plugin_info = {
    'enabled': True,
    'command_handle': ['ho:', 'thêm:', 'xem:', 'sửa:', 'xóa:', 'xóa_chọn:'],
